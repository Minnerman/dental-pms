...
def test_parse_charting_domains_accepts_temporary_notes():
    assert r4_import_script._parse_charting_domains_arg("temporary_notes") == ["temporary_notes"]


def test_cli_charting_canonical_temporary_notes_stats_shape(tmp_path, monkeypatch):
    class DummyStats:
        def as_dict(self):
            return {
                "created": 4,
    ...
    def fake_import(*_args, **kwargs):
        ...
        return DummyStats(), {
            ...
            "by_source": {"dbo.TemporaryNotes": {"fetched": 4}},
    ...
    assert captured["domains"] == ["temporary_notes"]
