...
def test_run_parity_treatment_plan_items_domain(monkeypatch, tmp_path: Path):
    def _build_with_data(*args, **kwargs):
        return {
            "patients": [
                {
    ...
    monkeypatch.setattr(r4_parity_run, "SessionLocal", lambda: _DummySession())
    monkeypatch.setattr(
        r4_parity_run.r4_treatment_plan_items_parity_pack,
        "build_parity_report",
        _build_with_data,
    )
    ...
    assert report["domain_summaries"]["treatment_plan_items"]["status"] == "pass"
    assert report["overall"]["status"] == "pass"
    assert (tmp_path / "treatment_plan_items.json").exists()


def test_run_parity_perio_plaque_domain(monkeypatch, tmp_path: Path):
    def _build_with_data(*args, **kwargs):
        return {
            "patients": [
                {
    ...
    assert report["domain_summaries"]["perio_plaque"]["status"] == "pass"
    assert report["overall"]["status"] == "pass"
    assert (tmp_path / "perio_plaque.json").exists()


def test_run_parity_chart_healing_actions_domain(monkeypatch, tmp_path: Path):
    def _build_with_data(*args, **kwargs):
        return {
            "patients": [
                {
    ...
    monkeypatch.setattr(r4_parity_run, "SessionLocal", lambda: _DummySession())
    monkeypatch.setattr(
        r4_parity_run.r4_chart_healing_actions_parity_pack,
        "build_parity_report",
        _build_with_data,
    )
    ...
    assert report["domain_summaries"]["chart_healing_actions"]["status"] == "pass"
    assert report["overall"]["status"] == "pass"
    assert (tmp_path / "chart_healing_actions.json").exists()


def test_run_parity_restorative_treatments_domain(monkeypatch, tmp_path: Path):
    def _build_with_data(*args, **kwargs):
        return {
            "patients": [
                {
    ...
    monkeypatch.setattr(r4_parity_run, "SessionLocal", lambda: _DummySession())
    monkeypatch.setattr(
        r4_parity_run.r4_restorative_treatments_parity_pack,
        "build_parity_report",
        _build_with_data,
    )
...
def test_run_parity_temporary_notes_domain(monkeypatch, tmp_path: Path):
    def _build_with_data(*args, **kwargs):
        return {
            "patients": [
                {
    ...
    monkeypatch.setattr(r4_parity_run, "SessionLocal", lambda: _DummySession())
    monkeypatch.setattr(
        r4_parity_run.r4_temporary_notes_parity_pack,
        "build_parity_report",
        _build_with_data,
    )
    ...
    assert report["domain_summaries"]["temporary_notes"]["status"] == "pass"
    assert report["overall"]["status"] == "pass"
    assert (tmp_path / "temporary_notes.json").exists()


def test_run_parity_appointment_notes_domain(monkeypatch, tmp_path: Path):
    def _build_with_data(*args, **kwargs):
        return {
            "patients": [
                {
    ...
    assert report["domain_summaries"]["appointment_notes"]["status"] == "pass"
    assert report["overall"]["status"] == "pass"
    assert (tmp_path / "appointment_notes.json").exists()


def test_run_parity_completed_questionnaire_notes_domain(monkeypatch, tmp_path: Path):
    def _build_with_data(*args, **kwargs):
        return {
            "patients": [
                {
    ...
    assert report["domain_summaries"]["completed_questionnaire_notes"]["status"] == "pass"
    assert report["overall"]["status"] == "pass"
    assert (tmp_path / "completed_questionnaire_notes.json").exists()


def test_run_parity_old_patient_notes_domain(monkeypatch, tmp_path: Path):
    def _build_with_data(*args, **kwargs):
        return {
            "patients": [
                {
    ...
    assert report["domain_summaries"]["old_patient_notes"]["status"] == "pass"
    assert report["overall"]["status"] == "pass"
    assert (tmp_path / "old_patient_notes.json").exists()


def test_run_parity_treatment_plans_domain(monkeypatch, tmp_path: Path):
    def _build_with_data(*args, **kwargs):
        return {
            "patients": [
                {
    ...
    monkeypatch.setattr(r4_parity_run, "SessionLocal", lambda: _DummySession())
    monkeypatch.setattr(
        r4_parity_run.r4_treatment_plans_parity_pack,
        "build_parity_report",
        _build_with_data,
    )
    ...
    assert report["domain_summaries"]["treatment_plans"]["status"] == "pass"
    assert report["overall"]["status"] == "pass"
