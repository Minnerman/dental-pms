...
from app.scripts import r4_temporary_notes_drop_report as drop_report
...
class _DummySource:
    ...
    def list_temporary_notes(self, **kwargs):
        return [
            R4TemporaryNote(
                patient_code=1001,
                source_row_id=100,
                note="Kept note",
                legacy_updated_at=datetime(2025, 1, 1, 9, 0, 0),
                user_code=9,
            ),
            R4TemporaryNote(
                patient_code=1001,
            ...
            R4TemporaryNote(
                ...
                source_row_id=102,
                note="Out of window",
                legacy_updated_at=datetime(2026, 2, 1, 0, 0, 0),
                user_code=9,
            ),
        ]
...
def test_build_drop_report_counts_sql_and_pg_and_reasons(monkeypatch):
    ...
    assert payload["sql_candidates_total"] == 4
    ...
    assert payload["sql_latest_key"]["source_id"] == "rowid:100"
