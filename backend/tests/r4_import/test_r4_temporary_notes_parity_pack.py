...
from app.scripts import r4_temporary_notes_parity_pack as pack
...
def test_sqlserver_rows_applies_blank_note_and_duplicate_filters_regression():
    class _Source:
        def list_temporary_notes(self, **_kwargs):
            return [
                ...
                R4TemporaryNote(
                    ...
                    source_row_id=21,
                    note="   ",
                    legacy_updated_at=datetime(2025, 1, 2, 10, 0, 0),
                    user_code=1,
                ),
                R4TemporaryNote(
                    patient_code=1001,
                    source_row_id=20,
                    note="dup source row",
                    legacy_updated_at=datetime(2025, 1, 2, 11, 0, 0),
                    user_code=1,
                ),
                R4TemporaryNote(
                    patient_code=1001,
    ...
    assert candidates == 4
    assert len(rows) == 1
    assert rows[0]["source_id"] == "rowid:20"
