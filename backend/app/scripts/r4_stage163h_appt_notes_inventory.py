...
def _view_has_required_columns(source: R4SqlServerSource) -> bool:
    try:
        ...
        note_col = source._pick_column("vwAppointmentDetails", ["notes"])  # noqa: SLF001
        return bool(patient_col and date_col and note_col)
    except Exception:
        return False
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Read-only Stage 163H scout inventory for appointment notes "
            "(prefers vwAppointmentDetails.notes, falls back to Appts.Notes)."
        )
    )
    ...
    if _view_has_required_columns(source):
        rows, column_metadata = query_note_rows(
            source,
            table="vwAppointmentDetails",
            patient_col_candidates=["patientcode"],
            date_col_candidates=["appointmentDateTimevalue"],
            note_col_candidates=["notes"],
            row_id_col_candidates=["apptid", "ApptID", "ApptId"],
        )
        source_preference = "vwAppointmentDetails"
    else:
        rows, column_metadata = query_note_rows(
            source,
            table="Appts",
            patient_col_candidates=["PatientCode"],
            date_col_candidates=["ApptDate", "StartTime", "StartDateTime", "ScheduledDate"],
            note_col_candidates=["Notes", "Note", "ApptNotes"],
            row_id_col_candidates=["ApptID", "ApptId", "AppointmentID", "AppointmentId", "ID", "Id"],
        )
        source_preference = "Appts"
    ...
    bundle = build_inventory_bundle(
        ...
        column_metadata={
            **column_metadata,
            "preferred_source_selected": source_preference,
        },
        date_from=date_from,
        date_to=date_to,
        seed=args.seed,
        cohort_limit=args.cohort_limit,
        recommended_seen_ledger=".run/seen_stage163h_appointment_notes.txt",
    )
    paths = write_standard_outputs(
        output_dir=Path(args.output_dir),
        filename_prefix="stage163h_appointment_notes",
        bundle=bundle,
    )
        ...
        json.dumps(
            {
                ...
                "source_preference_selected": source_preference,
