...
def _canonical_rows(
    ...
    *,
    ...
    stmt = (
        ...
        .where(
            R4ChartingCanonicalRecord.domain == "old_patient_note",
        ...
        patients.append(
            {
                ...
                "sqlserver_distinct_unique_keys": len({_note_unique_key(r) for r in sql_rows})
                if sql_source is not None
                else None,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build an OldPatientNotes parity pack from canonical data and SQL Server."
    )
