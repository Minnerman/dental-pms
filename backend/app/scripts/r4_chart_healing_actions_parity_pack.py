...
def _latest_sort_tuple(row: dict[str, object]) -> tuple[str, int]:
    return (str(row.get("recorded_at") or ""), int(row.get("action_id") or 0))


def _unique_key(row: dict[str, object]) -> str:
    return f"{row.get('patient_code')}:{row.get('action_id')}"


def _latest_key(row: dict[str, object] | None) -> dict[str, object] | None:
    if row is None:
        return None
    return {"recorded_at": row.get("recorded_at"), "action_id": row.get("action_id")}
...
def _canonical_rows(
    ...
    *,
    ...
    stmt = (
        ...
        .where(
            R4ChartingCanonicalRecord.domain == "chart_healing_action",
    ...
    for row in session.execute(stmt).scalars().all():
        ...
        if action_id is None and row.r4_source_id:
            try:
                action_id = int(str(row.r4_source_id))
            except ValueError:
                action_id = None
        out.append(
            {
...
def _sqlserver_rows(
    ...
    *,
    ...
    for item in source.list_chart_healing_actions(
        ...
        if not _in_date_window(item.action_date, date_from, date_to):
            continue
        out.append(
            {
                "patient_code": item.patient_code,
                "action_id": item.action_id,
                "recorded_at": _iso(item.action_date),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a ChartHealingActions parity pack from canonical data and SQL Server."
    )
