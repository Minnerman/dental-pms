...
def _plaque_unique_key(row: dict[str, object]) -> str:
    return f"{row.get('trans_id')}:{row.get('tooth')}"
...
def _digest_for_trans(rows: list[dict[str, object]], trans_id: int | None) -> list[dict[str, object]]:
    ...
    matched.sort(key=lambda r: int(r.get("tooth") or 0))
    return matched
...
def _canonical_rows(
    ...
    *,
    ...
    stmt = (
        ...
        .where(
            R4ChartingCanonicalRecord.domain == "perio_plaque",
...
def _sqlserver_rows(
    ...
    *,
    ...
    for item in source.list_perio_plaque(
        ...
        if not _in_date_window(item.recorded_at, date_from, date_to):
            continue
        out.append(
            {
...
def build_parity_report(
    ...
    *,
    ...
    for code in patient_codes:
        ...
        canonical_digest = _digest_for_trans(
            canonical,
            canonical_latest_key.get("trans_id") if canonical_latest_key else None,
        )
        sql_digest = _digest_for_trans(
            sql_rows,
            sql_latest_key.get("trans_id") if sql_latest_key else None,
        )
        ...
        patients.append(
            {
                ...
                "canonical_distinct_unique_keys": len({_plaque_unique_key(r) for r in canonical}),
                "sqlserver_total_rows": len(sql_rows) if sql_source is not None else None,
                "sqlserver_distinct_unique_keys": (
                    len({_plaque_unique_key(r) for r in sql_rows}) if sql_source is not None else None
                ),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build a PerioPlaque parity pack from canonical data and (optionally) SQL Server."
    )
