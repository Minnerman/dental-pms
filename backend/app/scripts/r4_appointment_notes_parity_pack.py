...
from app.services.r4_charting.appointment_notes_import import (
    appointment_note_source_id,
    filter_appointment_notes,
)
from app.services.r4_import.sqlserver_source import R4SqlServerConfig, R4SqlServerSource

_DOMAIN_NAMES = ("appointment_note", "appointment_notes")
...
def _latest_sort_tuple(row: dict[str, object]) -> tuple[str, int, str]:
    return (
        ...
        int(row.get("source_apptid") or 0),
        ...
        rows.append(
            {
                ...
                "source_apptid": payload.get("source_apptid"),
                "status": payload.get("status"),
                "appt_flag": payload.get("appt_flag"),
...
def _sqlserver_rows(
    ...
    *,
    ...
    candidates = list(
        source.list_appointment_notes(
            patients_from=patient_code,
            patients_to=patient_code,
            limit=row_limit,
        )
    )
    accepted, dropped = filter_appointment_notes(
        candidates,
        date_from=date_from,
        date_to=date_to,
    )
        ...
        rows.append(
            {
                ...
                "recorded_at": _iso(item.appointment_datetime),
                "source_id": appointment_note_source_id(item),
...
def build_parity_report(
    ...
    *,
    ...
    latest_total = sum(1 for patient in patients if isinstance(patient.get("latest_match"), bool))
    ...
    if not include_sqlserver:
        ...
        status = "pass" if latest_matched == latest_total and digest_matched == digest_total else "fail"
    else:
        status = "fail"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build an appointment_notes parity pack from canonical data and SQL Server."
    )
