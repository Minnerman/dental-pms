...
test("appointments deep link preserves week view in calendar mode", async ({
  ...
  await openAppointments(page, request, `/appointments?date=${targetDate}&view=week`);

  const jumpDateInput = page.getByTestId("appointments-jump-date-input");
  const calendarModeButton = page.getByTestId("appointments-view-calendar");
  const calendarToolbar = page.getByTestId("appointments-calendar-toolbar");
  const weekViewButton = page.getByTestId("appointments-calendar-view-week");

  await expect(jumpDateInput).toHaveValue(targetDate);
  await expect(calendarModeButton).toHaveClass(/btn btn-primary/);
  await expect(calendarToolbar).toBeVisible({ timeout: 15_000 });
...
test("appointments deep link prefills start time in booking flow", async ({ page, request }) => {
  const startValue = "2026-01-14T13:30";

  await openAppointments(page, request, `/appointments?book=1&start=${startValue}`);
  await expect(page.getByTestId("booking-modal")).toBeVisible({ timeout: 15_000 });
...
test("appointments deep link converts UTC start to local time in booking flow", async ({
  page,
  request,
}) => {
  const startValue = "2026-01-14T13:30:00Z";

  await openAppointments(page, request, `/appointments?book=1&start=${startValue}`);
  await expect(page.getByTestId("booking-modal")).toBeVisible({ timeout: 15_000 });

  const expectedLocalStart = await page.evaluate((value) => {
  ...
  const startInput = page.getByTestId("booking-start");
  await expect(startInput).toHaveValue(expectedLocalStart);
...
test("appointments deep link ignores invalid start safely in booking flow", async ({
  page,
  request,
}) => {
  await openAppointments(page, request, "/appointments?book=1&start=not-a-date");
  await expect(page.getByTestId("booking-modal")).toBeVisible({ timeout: 15_000 });
