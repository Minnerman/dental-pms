...
test("patient charting review pack falls back to backend-contract filename without header", async ({
  page,
  request,
}) => {
  const baseUrl = getBaseUrl();
  ...
  const patientId = await createPatient(request, {
    first_name: "Charting",
    last_name: "ReviewPackFallback",
  });
  const legacyPatientCode = 1012456;
  ...
  await page.route(metaRoutePattern, async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        patient_id: patientId,
        legacy_patient_code: legacyPatientCode,
        last_imported_at: "2024-07-05T10:00:00Z",
        source: "r4",
      }),
    });
  });
  ...
  await page.route(routePattern, async (route) => {
    await route.fulfill({
      ...
      body: Buffer.from("PK\x03\x04charting-review-pack-fallback"),
    });
  });

  const downloadPromise = page.waitForEvent("download");
  await reviewPackButton.click();
  const download = await downloadPromise;
  expect(download.suggestedFilename()).toBe(expectedFilename);
