...
test("selected invoice latest receipt shows an error state and allows retry", async ({
  page,
  request,
}) => {
  const patientId = await createPatient(request, {
    first_name: "Billing",
    last_name: `Receipt Retry ${Date.now()}`,
  });
  const invoice = await createInvoice(request, patientId);
  await addInvoiceLine(request, invoice.id, {
    description: "Latest receipt retry proof",
    quantity: 1,
    unit_price_pence: 2500,
  });
  const issued = await issueInvoice(request, invoice.id);
  ...
  const expectedFilename = `receipt-${invoice.id}-${payment.id}.pdf`;
  const routePattern = new RegExp(`/api/payments/${payment.id}/receipt\\.pdf$`);
  const errorMessage = "Receipt download unavailable";
  ...
  let seenRetryRequest!: () => void;
  const seenRetryRequestPromise = new Promise<void>((resolve) => {
    seenRetryRequest = resolve;
  });
  let releaseRetryResponse!: () => void;
  const releaseRetryResponsePromise = new Promise<void>((resolve) => {
  ...
  await page.route(routePattern, async (route) => {
    seenRetryRequest();
    await releaseRetryResponsePromise;
    await route.fulfill({
      status: 200,
      headers: {
        "Content-Type": "application/pdf",
