...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try { setCollapsed(localStorage.getItem(sidebarStorageKey) === "true"); } catch { /* Optional preference. */ }
        ...
        <div className="app-sidebar-scroll">
          ...
          {moreTabs.length > 0 && <div className="app-sidebar-more">
            <button className={`app-sidebar-link${moreTabs.some((tab) => isActive(tab.href)) ? " active" : ""}`} aria-label="More"
              ...
              onClick={() => { if (isCollapsed) { setCollapsed(false); setMoreOpen(true); } else setMoreOpen(!moreOpen); }}>
