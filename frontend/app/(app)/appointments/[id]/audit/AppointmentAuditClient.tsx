...
export default function AppointmentAuditClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await apiFetch(`/api/appointments/${appointmentId}/audit`);
  ...
  return (
    <div className="app-grid">
      ...
      {loading ? (
        ...
        <AuditTable title="Appointment audit" rows={rows} />
      )}
      {status && status >= 500 && (
