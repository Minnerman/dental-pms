...
- `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_PACKAGE.md` records the bounded fixture package, `docs/r4/fixtures/opening_balance_bounded_fixture/APPROVAL_RECORD_20260507.md` records owner approval for the exact fixture hash and manifest checksum, `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_EXECUTION_READINESS.md` records execution-readiness verification, and `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_VALIDATION_NOWRITE_EVIDENCE.md` records validation/no-write evidence. The validation/no-write evidence did not use `--apply` or `--confirm`, did not connect to a PMS database, and did not create rows or finance records; guarded apply/write still needs separate explicit authorisation and must keep `finance_import_ready=false` with finance import out of scope.
...
1. Review bounded fixture validation/no-write evidence before any guarded apply/write decision.
   - Target: review `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_VALIDATION_NOWRITE_EVIDENCE.md`, then decide whether to separately authorise a scratch/test-only guarded apply/write proof.
   - Why next: validation/no-write has passed against the approved fixture without `--apply`, `--confirm`, PMS DB connection, ledger rows, or finance records. Apply/write is still not authorised.
