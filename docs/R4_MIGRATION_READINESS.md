...
- SELECT-only finance source discovery and inventory are complete as of PR #586 and PR #587. Finance sign/cancellation/allocation policy is recorded in `docs/r4/R4_FINANCE_SIGN_CANCELLATION_ALLOCATION_POLICY.md`, PR #591 completed the backend-only pure finance classification/sign helper proof, PR #593 completed backend-only SELECT-only opening balance reconciliation command/report tooling, the 2026-05-04 live opening balance proof evidence is recorded, PR #596 completed live SELECT-only cancellation/refund/allocation proof evidence, and `docs/r4/R4_FINANCE_REFUND_ALLOCATION_CHARGE_REF_DECISION.md` records the refund-allocation/charge-ref semantics decision without importer wiring, PMS DB writes, R4 writes, or finance record changes.
...
1. SELECT-only cash-event staging proof from `vwPayments` and `Adjustments`.
   - Target: produce candidate/excluded/manual-review counts for active payment, refund, and credit cash-event rows without importing anything.
   - Why next: `docs/r4/R4_FINANCE_REFUND_ALLOCATION_CHARGE_REF_DECISION.md` keeps allocations reconciliation-only, blocks invoice application because charge refs are `0` / missing `3130`, and selects `vwPayments` plus `Adjustments` as the safest next proof source.
   - Likely files: `backend/app/services/r4_import/finance_cash_event_staging.py`, `backend/app/scripts/r4_finance_cash_event_staging.py`, and `backend/tests/r4_import/test_r4_finance_cash_event_staging.py`; no importer, staging model, or PMS DB session.
   - Likely validation: focused proof/report tests, existing finance classification helper tests, existing cancellation/allocation proof tests if reused, optional live SELECT-only JSON/stdout/stderr artefact if R4 env is available; no R4 writes, no PMS DB writes, no finance records created or changed.
...
5. Finance payment method mapping helper continuation.
   - Target: after cash-event staging has a stable candidate/exclusion shape, tighten payment method and lookup mapping using the PR #591 classifier reason codes.
   - Why next later: payment method mapping matters for cash-up and reports, but cash-event candidate/exclusion proof is the higher-risk blocker.
...
- Refund-allocation/charge-ref semantics are recorded in `docs/r4/R4_FINANCE_REFUND_ALLOCATION_CHARGE_REF_DECISION.md`: allocations remain reconciliation-only, missing charge refs block invoice application, and `vwPayments` plus `Adjustments` are the next proof sources for cash-event candidates.
- Next, run a SELECT-only cash-event staging proof before any importer, staging model, or PMS finance write path.
