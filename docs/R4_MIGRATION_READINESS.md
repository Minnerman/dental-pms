...
1. Run and record live SELECT-only cash-event staging proof from `vwPayments` and `Adjustments`.
   - Target: execute the PR #599 merged proof/report command to produce candidate/excluded/manual-review counts for active payment, refund, and credit cash-event rows without importing anything.
   - Why next: the tooling is merged, but live evidence remains pending because complete R4 env was unavailable during PR #599.
   - Likely files: no implementation files; capture JSON/stdout/stderr artefacts under `.run/` and follow with a docs/evidence refresh if the run succeeds.
   - Likely validation: confirm `R4_SQLSERVER_ENABLED=true`, `R4_SQLSERVER_READONLY=true`, R4 host/user/password present, command exit `0`, JSON `select_only=true`, no R4 writes, no PMS DB writes, no finance records created or changed.
...
5. Finance payment method mapping helper continuation.
   - Target: after live cash-event staging has a stable candidate/exclusion shape, tighten payment method and lookup mapping using the PR #591 classifier reason codes.
...
- PR #599 added backend-only SELECT-only cash-event staging proof tooling; next, run and record the live SELECT-only proof when complete R4 env is available before any importer, staging model, or PMS finance write path.
