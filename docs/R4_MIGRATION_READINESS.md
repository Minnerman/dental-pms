...
1. Backend-only opening-balance dry-run/report CLI plus tests, no writes.
   - Target: implement the no-write report shape from `docs/r4/R4_FINANCE_OPENING_BALANCE_SCRATCH_DRYRUN_DESIGN.md` by composing PR #604's pure plan helper with PatientStats source evidence and patient mapping evidence.
   - Why next: the deterministic row planner and scratch dry-run/report design are now complete; the smallest executable proof is a report-only CLI that proves mapping summaries, refusal reasons, safety gates, and artefact shape before any scratch apply design.
   - Likely files: `backend/app/services/r4_import/opening_balance_snapshot_dry_run.py`, `backend/app/scripts/r4_opening_balance_snapshot_dry_run.py`, and `backend/tests/r4_import/test_opening_balance_snapshot_dry_run.py`.
   - Likely validation: focused unit tests for report shape and guards, existing opening-balance snapshot plan helper tests, and `git diff --check`; no live R4 query unless explicitly capturing SELECT-only evidence; no R4 writes; no PMS DB writes; no finance records created or changed.
   - Backend-only/proof-only likely: yes; no importer wiring, staging model, or write path.
   - Risk: high.
...
- PR #604 completed the backend-only pure opening-balance snapshot plan helper proof. The next finance slice is backend-only opening-balance dry-run/report CLI plus tests, with no importer, staging model, scratch write, or PMS finance write path.
