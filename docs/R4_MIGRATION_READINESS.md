...
1. Guarded scratch-only opening-balance apply execution proof, only after explicit instruction.
   - Target: use the new guarded CLI prototype only against an explicitly isolated scratch/test PMS DB, after a full eligible-row artefact or deliberately bounded scratch fixture is available.
   - Why next: the CLI prototype proves command/guard/write mechanics in tests, but no preserved-evidence scratch execution has started and the existing preserved dry-run report has bounded eligible samples.
   - Likely files: docs/evidence only unless a full eligible-row dry-run artefact export helper is explicitly authorised.
   - Likely validation: validation mode first, default/live refusal proof, scratch-only apply, idempotency rerun, invoice/payment counts unchanged, stdout/stderr/exit-code/report artefacts, no R4 access, no live/default PMS writes, and no finance import.
   - Backend-only/proof-only likely: execution proof should avoid new implementation unless the full-row artefact blocker is explicitly chosen.
   - Risk: high.
