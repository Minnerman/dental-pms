...
1. Completed questionnaire / old patient notes first scale-out proof.
   ...
   - Likely validation: patients import for selected cohort, `charting_canonical` apply/rerun stats, `r4_parity_run` for both domains, drop/explain samples.
...
- Newer canonical note lanes still need scale-out parity proof.
...
- Scale recently wired note domains using the existing deterministic cohort and parity pattern.
