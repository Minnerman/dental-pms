...
Baseline: `origin/master@c42bc48522842711a90fa6239ca0dcd88315625f`
...
1. Decide whether to separately authorise a bounded fixture guarded apply/write proof.
   - Target: use the guarded apply/write readiness report `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_GUARDED_APPLY_READINESS.md`, signed-off validation/no-write evidence in `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_VALIDATION_NOWRITE_EVIDENCE.md`, and sign-off record `docs/r4/fixtures/opening_balance_bounded_fixture/VALIDATION_NOWRITE_SIGNOFF_20260508.md` to decide whether to separately authorise a scratch/test-only guarded apply/write proof.
   - Why next: validation/no-write has passed, owner sign-off is recorded for the approved fixture without `--apply`, `--confirm`, PMS DB connection, ledger rows, or finance records, and readiness prerequisites are documented for consideration. Apply/write is still not authorised.
   - Likely files: if authorised later, a docs/evidence record for guarded scratch apply/idempotency proof; backend changes are not expected.
   - Likely validation: recheck manifest/checksum/expected-total/eligible-count/repo-SHA, default/live refusal proof, isolated scratch/test-only apply only if explicitly authorised, idempotency rerun, invoice/payment counts unchanged, stdout/stderr/exit-code/report artefacts, no real patient data in committed docs, no R4 access unless separately authorised as SELECT-only, no live/default PMS writes, and no finance import.
