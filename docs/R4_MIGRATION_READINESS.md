...
- SELECT-only finance source discovery and inventory are complete as of PR #586 and PR #587. Finance sign/cancellation/allocation policy is recorded in `docs/r4/R4_FINANCE_SIGN_CANCELLATION_ALLOCATION_POLICY.md`, PR #591 completed the backend-only pure finance classification/sign helper proof, PR #593 completed backend-only SELECT-only opening balance reconciliation command/report tooling, the 2026-05-04 live opening balance proof evidence is recorded, PR #596 completed live SELECT-only cancellation/refund/allocation proof evidence, `docs/r4/R4_FINANCE_REFUND_ALLOCATION_CHARGE_REF_DECISION.md` records the refund-allocation/charge-ref semantics decision, the 2026-05-04 live cash-event staging proof evidence is complete with `finance_import_ready=false`, and `docs/r4/R4_FINANCE_INVOICE_CHARGE_REF_SOURCE_DECISION.md` records that no explicit patient invoice/statement source or usable allocation charge refs are proven, all without importer wiring, PMS DB writes, R4 writes, or finance record changes.
...
1. Opening-balance snapshot import design/proof.
   - Target: decide whether a bounded opening-balance snapshot path can avoid historical invoice reconstruction while preserving R4 raw balance evidence and avoiding double counting.
   - Why next: live invoice/charge-ref source discovery found no explicit patient invoice/statement source and no usable allocation charge refs; live cash-event proof remains `finance_import_ready=false`, while PatientStats component checks already passed with `0` mismatches.
   - Likely files: docs/design-only decision first; backend proof-only helper/report only if inspection proves a narrow executable proof is required.
   - Likely validation: `git diff --check`; no R4 writes; no PMS DB writes; no finance records created or changed.
   - Backend-only/proof-only likely: no by default; docs/design-only should be tried first.
   - Risk: high.
...
5. Finance payment method mapping helper continuation.
   - Target: after the import-readiness decision selects payment method mapping as the next blocker, tighten R4 payment and lookup mapping using the live cash-event method-family evidence and PR #591 classifier reason codes.
   - Why next later: payment method mapping matters for cash-up and reports, but it should follow the decision on whether charge-ref/invoice source discovery is the higher-priority blocker.
   - Likely files: backend pure helper/report tests or docs policy update, depending on reconciliation results.
   - Likely validation: focused unit/report tests, no R4 writes, no PMS DB writes.
...
- Finance classification/sign helper proof is complete as of PR #591, opening balance reconciliation command/report tooling is complete as of PR #593, live opening balance proof evidence is recorded as of 2026-05-04, and cancellation/refund/allocation reconciliation proof evidence is complete as of PR #596.
...
- Next finance slice is a payment method mapping / import-readiness decision or charge-ref / invoice source discovery decision before any importer, staging model, or PMS finance write path.
