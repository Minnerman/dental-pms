...
| Past appointments | Yes: `dbo.vwAppointmentDetails`; live SELECT-only inventory records 101,051 appointments from 2001-10-27 to 2027-02-01, with 100,994 before the 2026-04-29 cutover date. | Yes: `r4_import --entity appointments` into `r4_appointments`. | Strong for raw R4 staging and promotion planning: linkage report/queue, manual mappings, unmapped appointments admin, live inventory evidence, PR #571 pure helper proof, the 2026-04-30 scratch transcript mapped `99299` rows with `1752` expected null-patient rows and `0` actionable unmapped rows, PR #574's no-core-write promotion dry-run/report classified the full `101051` rows, and PR #576 added the pure promotion plan helper/proof. | Strong for raw `r4_appointments` and no-core-write promotion classification: scratch import created `101051`, idempotency rerun created `0` and skipped `101051`; PR #574 found `94156` status-policy promote candidates and kept core `appointments` unchanged at `0` before/after; PR #576 made the row-level plan consumable without DB writes. | Partial: read-only R4 calendar/admin linking plus core appointments UI. | Timezone/local-time proof before conflict predicate extraction and before any guarded core-promotion apply path. | High | Medium-large |
| Future appointments | Yes: same `dbo.vwAppointmentDetails`; live inventory records 57 rows on/after 2026-04-29, including `Cancelled`, `Pending`, and `Deleted` examples through 2027-02-01. | Yes: same appointment importer. | Strong for no-core-write promotion planning: live inventory evidence, PR #571 fail-closed status helper tests, full scratch raw import/linkage evidence, PR #574 promotion dry-run evidence, and PR #576 promotion plan helper/proof exist; future core diary write semantics remain unproven. | Strong for raw `r4_appointments` and no-core-write reporting: PR #574 classified `47` future booked candidates, `7` future cancelled candidates, and `3` future deleted excluded rows without core writes; PR #576 proves deterministic row-level planning without importer/apply wiring. | Partial: core diary, booking, R4 calendar read-only. | Timezone/local-time proof for R4 appointment local times and daylight-saving boundaries before conflict predicate extraction or active future diary promotion. | High | Medium-large |
...
4. R4 appointment timezone/local-time proof, using the PR #574 no-core-write report and PR #576 promotion plan helper as input evidence before conflict predicate extraction or any core diary write path is enabled.
...
1. Appointment timezone/local-time proof.
   - Target: prove how R4 appointment local clinic times should map to PMS timezone-aware core diary timestamps before any conflict predicate extraction or apply path.
   - Why next: raw import, no-core-write promotion reporting, and PR #576's pure promotion plan helper are complete; timezone/local-time and daylight-saving semantics are the highest-risk unresolved prerequisite for future diary rows.
   - Likely files: narrowly scoped backend pure helper/tests, or docs/design plus tests if inspection shows an existing helper can be reused; do not enable live/default-DB core `appointments` writes.
   - Likely validation: focused local-time/DST fixtures, existing status/promotion plan tests, git diff checks, no R4 writes, no live/default PMS DB writes.
