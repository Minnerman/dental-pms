...
- SELECT-only finance source discovery and inventory are complete as of PR #586 and PR #587. Finance sign/cancellation/allocation policy is recorded in `docs/r4/R4_FINANCE_SIGN_CANCELLATION_ALLOCATION_POLICY.md`, PR #591 completed the backend-only pure finance classification/sign helper proof, PR #593 completed backend-only SELECT-only opening balance reconciliation command/report tooling, the 2026-05-04 live opening balance proof evidence is recorded, PR #596 completed live SELECT-only cancellation/refund/allocation proof evidence, `docs/r4/R4_FINANCE_REFUND_ALLOCATION_CHARGE_REF_DECISION.md` records the refund-allocation/charge-ref semantics decision, and the 2026-05-04 live cash-event staging proof evidence is complete with `finance_import_ready=false`, all without importer wiring, PMS DB writes, R4 writes, or finance record changes.
...
1. Payment method mapping / import-readiness decision or charge-ref / invoice source discovery decision.
   - Target: decide whether the next finance blocker is payment method/import-readiness policy from the live cash-event proof or explicit charge-ref/invoice source discovery.
   - Why next: the live cash-event proof is complete and reports `finance_import_ready=false`; import remains blocked by proof-only payment method mapping, unresolved allocation refunds, absent charge refs, manual-review rows, and no explicit invoice/statement/charge-ref source.
