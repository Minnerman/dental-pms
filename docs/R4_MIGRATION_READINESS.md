...
| Finance ledger | Yes: PR #586 identified `dbo.PatientStats`, `dbo.vwPayments`, `dbo.Adjustments`, `dbo.Transactions`, `dbo.PaymentAllocations`, `dbo.vwAllocatedPayments`, lookup tables, and scheme/classification sources; PR #587 added repeatable SELECT-only inventory evidence. | No R4 finance importer found. | No import reconciliation yet; PR #587 inventory records balance/payment/adjustment/allocation counts and key risks, `docs/r4/R4_FINANCE_SIGN_CANCELLATION_ALLOCATION_POLICY.md` records the fail-closed policy, PR #591 added the pure classification/sign helper proof, PR #593 added SELECT-only opening balance reconciliation tooling, the 2026-05-04 live opening balance proof recorded PatientStats component consistency and cross-source indicators, PR #596 added live cancellation/refund/allocation evidence, `docs/r4/R4_FINANCE_REFUND_ALLOCATION_CHARGE_REF_DECISION.md` records the refund-allocation/charge-ref decision, PR #599 added cash-event staging proof tooling, the 2026-05-04 live cash-event proof reported `finance_import_ready=false`, `docs/r4/R4_FINANCE_INVOICE_CHARGE_REF_SOURCE_DECISION.md` records the invoice/charge-ref source decision, `docs/r4/R4_FINANCE_OPENING_BALANCE_SNAPSHOT_DESIGN.md` records the opening-balance snapshot design, PR #604 added the pure opening-balance snapshot plan helper proof, `docs/r4/R4_FINANCE_OPENING_BALANCE_SCRATCH_DRYRUN_DESIGN.md` records the scratch-only dry-run/report design, PR #607 added no-write opening-balance dry-run/report tooling, the 2026-05-05 scratch-only dry-run execution evidence proved all `1018/1018` non-zero candidates mapped with `import_ready=false`, the scratch mapping stack cleanup removed the isolated stack while preserving artefacts, `docs/r4/R4_FINANCE_OPENING_BALANCE_GUARDED_APPLY_DESIGN.md` records the guarded scratch-only apply design, PR #612 added the pure guarded apply planning/preflight helper proof, PR #614 added the guarded scratch-only apply CLI prototype, PR #615 added a bounded synthetic scratch execution proof using local SQLite `tmp_path` data only, PR #616 records the future preserved-evidence scratch execution plan, `docs/r4/R4_FINANCE_OPENING_BALANCE_EXECUTION_PACKAGE_DECISION.md` recommends an approved bounded fixture as the next execution package, PR #618 prepared the bounded fixture package, `docs/r4/fixtures/opening_balance_bounded_fixture/APPROVAL_RECORD_20260507.md` records owner approval for that exact package hash/checksum for future scratch/test-only use, `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_EXECUTION_READINESS.md` records execution-readiness verification, `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_VALIDATION_NOWRITE_EVIDENCE.md` records no-write validation evidence, `docs/r4/fixtures/opening_balance_bounded_fixture/VALIDATION_NOWRITE_SIGNOFF_20260508.md` records sign-off for that no-write evidence, `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_GUARDED_APPLY_EVIDENCE.md` records the first local isolated scratch/test guarded apply/idempotency proof, `docs/r4/fixtures/opening_balance_bounded_fixture/GUARDED_APPLY_EVIDENCE_SIGNOFF_20260508.md` records owner sign-off for that proof evidence only, `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_COMPLETION_SUMMARY.md` records bounded-fixture pathway completion, and `docs/r4/R4_FINANCE_OPENING_BALANCE_FULL_ELIGIBLE_ARTEFACT_PLAN.md` starts the full eligible-row artefact planning track without artefact access or execution. | Inventory, policy, helper proof, opening balance report tooling, live opening balance proof evidence, live cancellation/allocation proof evidence, semantics decisions, cash-event staging proof tooling, live cash-event proof evidence, invoice/charge-ref decision, opening-balance snapshot design, pure plan helper proof, scratch dry-run/report design, no-write dry-run/report tooling, scratch-only dry-run execution evidence, cleanup evidence, guarded apply design, guarded apply planning/preflight helper proof, guarded apply CLI prototype tests, bounded synthetic scratch proof, preserved-evidence execution planning, package decision, bounded fixture package, owner approval record, execution-readiness verification, no-write validation evidence, no-write sign-off, first bounded guarded apply/idempotency proof, proof evidence sign-off, bounded-fixture completion summary, and full eligible-row artefact planning only: no live finance records written. | Yes: PMS patient ledger, payments/adjustments, cash-up/outstanding/trends reports. | The approved bounded fixture has a local isolated scratch/test guarded apply proof with `created=3`, idempotency proof with `created=0` and `skipped=3`, and owner sign-off for that proof evidence; the full eligible-row artefact track is planning-only and remains blocked pending artefact provenance, redaction/storage approval, manifest/checksum/expected-total approval, scratch/test target approval, validation/no-write authorisation, evidence sign-off, separate guarded apply/write authorisation, and proof evidence sign-off. Finance import remains out of scope. | High | Large |
...
| Balances | Yes: `dbo.PatientStats` is the high-confidence balance/aged-debt snapshot source. | No R4 balance importer found. | No balance import reconciliation yet; PR #587 found `17012` PatientStats rows, `1018` non-zero balances, total balance `-131342.13`, and aged debt 30-60 `379.84`, 60-90 `579.00`, 90+ `9370.98`; PR #593 added the SELECT-only opening balance reconciliation command/report tooling; the 2026-05-04 live proof found Balance component checks passed with `0` mismatches, TreatmentBalance split checks passed with `0` mismatches, aged debt total `10329.82`, and `892` non-zero-balance rows with no aged debt; the opening-balance snapshot design keeps components/aged debt as metadata and recommends one future manifest-scoped adjustment row per eligible non-zero mapped patient in scratch only; PR #604 added the pure plan helper proof for eligibility, signs, pence conversion, patient mapping, and fail-closed decisions; the scratch dry-run/report design defines the no-write report shape, safety gates, mapping success criteria, and artefacts before any apply design; PR #607 added backend-only dry-run/report tooling with JSON inputs, no R4 source mode, no DB write path, and no apply mode; the 2026-05-05 scratch execution used real PatientStats rows plus scratch mappings and produced `eligible=1018`, `no_op=15820`, `component_mismatch=174`, `import_ready=false`; scratch cleanup later removed the isolated scratch stack and preserved artefacts; the guarded apply design selects existing `PatientLedgerEntry` adjustment rows for a future scratch-only prototype; PR #612 proves the no-write planning/preflight gates; PR #614 added the guarded scratch-only CLI prototype and tests scratch-only `PatientLedgerEntry` adjustment creation/idempotency against local scratch-test SQLite; PR #615 added a bounded synthetic CLI execution proof with two generated non-R4 rows; PR #616 planned preserved-evidence scratch execution; the execution package decision recommends an approved bounded fixture next. | Inventory, report tooling, live proof evidence, semantics decisions, cash-event tooling, live cash-event evidence, opening-balance design, pure plan helper proof, scratch dry-run/report design, no-write dry-run/report tooling, scratch-only dry-run execution evidence, cleanup evidence, guarded apply design, guarded apply planning/preflight helper proof, guarded apply CLI prototype tests, bounded synthetic scratch proof, preserved-evidence execution planning, and package decision only: no live/default balance records written. | Yes: PMS patient balance and outstanding reports from ledger. | Synthetic CLI proof is not a full `1018`-row scratch execution proof; preserved bounded-sample dry-run evidence needs an approved bounded fixture for the next scratch/test-only proof, while a full eligible-row artefact package remains blocked pending provenance and approval. | High | Medium |
...
- `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_PACKAGE.md` records the bounded fixture package, `docs/r4/fixtures/opening_balance_bounded_fixture/APPROVAL_RECORD_20260507.md` records owner approval for the exact fixture hash and manifest checksum, `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_EXECUTION_READINESS.md` records execution-readiness verification, `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_VALIDATION_NOWRITE_EVIDENCE.md` records validation/no-write evidence, `docs/r4/fixtures/opening_balance_bounded_fixture/VALIDATION_NOWRITE_SIGNOFF_20260508.md` records owner sign-off for that validation/no-write evidence only, `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_GUARDED_APPLY_READINESS.md` records guarded apply/write readiness prerequisites for consideration only, `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_GUARDED_APPLY_EVIDENCE.md` records the first local isolated scratch/test guarded apply/idempotency proof, `docs/r4/fixtures/opening_balance_bounded_fixture/GUARDED_APPLY_EVIDENCE_SIGNOFF_20260508.md` records owner sign-off for that proof evidence only, and `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_COMPLETION_SUMMARY.md` records the final post-#625 bounded-fixture pathway status. The apply proof used `--apply`, exact `--confirm SCRATCH_OPENING_BALANCE_APPLY`, and `--actor-id`; first run created `3` scratch/test `PatientLedgerEntry` rows, second run created `0` and skipped `3`, invoices/payments stayed `0`, and `finance_import_ready=false`. No live/default PMS DB, actual PMS Postgres, R4, real artefact, real patient data, finance import, production execution, or full eligible-row artefact execution was authorised by the sign-off or completion summary.
- `docs/r4/R4_FINANCE_OPENING_BALANCE_FULL_ELIGIBLE_ARTEFACT_PLAN.md` starts the later full eligible-row artefact planning track, and `docs/r4/R4_FINANCE_OPENING_BALANCE_FULL_ELIGIBLE_ARTEFACT_PROVENANCE_REDACTION_PROPOSAL.md` proposes provenance/redaction governance for a future artefact package. Both are planning-only: no R4 access, real artefact access, patient data, PMS DB connection, guarded scratch apply, CLI validation/no-write, live/default PMS write, actual PMS Postgres write, finance import, or full eligible-row artefact execution occurred. They define future package requirements, redaction/storage rules, approval gates, validation/no-write evidence, guarded apply/write evidence, rejection criteria, and stop conditions while keeping `finance_import_ready=false`.
...
1. Full eligible-row opening-balance artefact plan and provenance/redaction review.
   - Target: `docs/r4/R4_FINANCE_OPENING_BALANCE_FULL_ELIGIBLE_ARTEFACT_PLAN.md` and `docs/r4/R4_FINANCE_OPENING_BALANCE_FULL_ELIGIBLE_ARTEFACT_PROVENANCE_REDACTION_PROPOSAL.md`.
   - Why next: the bounded fixture package, approval, execution-readiness verification, validation/no-write evidence, validation/no-write sign-off, guarded apply/write readiness check, guarded apply/idempotency proof, guarded apply proof evidence sign-off, and completion summary are complete for manifest `ob-bounded-fixture-20260507-000001` only; the full eligible-row artefact path still needs owner review of provenance, redaction/storage policy, manifest/checksum/expected-total gates, validation/no-write gates, and later scratch/test-only proof gates before any execution can be considered.
   - Likely files: docs/status or a later owner sign-off/provenance approval only.
   - Likely validation: fixture/manifest hash validation and docs diff checks only; no real artefact access, no R4 access, no guarded apply, no CLI validation/no-write rerun, no PMS DB connection, and no finance import.
   - Backend-only/proof-only likely: docs-only if any work is requested.
   - Risk: low for plan review; high for any attempt to proceed beyond planning without the required approvals.
...
5. Non-finance full-dry-run sequencing runbook.
   - Target: turn the recommended non-finance import order into a restartable isolated dry-run sequence before any finance or future-diary write work.
   - Why next: patients, users, treatment codes, treatment plans/items, treatment transactions, and charting have more mature import evidence than finance and can be planned without touching live/default data.
   - Likely files: docs/runbook or proof manifest only.
   - Likely validation: docs diff checks, existing importer test references, no R4 writes.
   - Backend-only/docs-only: likely yes.
   - Risk: medium.
