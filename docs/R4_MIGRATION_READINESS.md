...
Baseline: `master@719cb78c6b85f39720cc38daf1b509d83abd335a`
...
1. Scratch-only opening-balance dry-run execution evidence, no writes.
   - Target: run PR #607's no-write dry-run/report tooling against the real PatientStats row artefact and scratch patient mapping evidence, then capture JSON/stdout/stderr/exit-code artefacts.
   - Why next: the deterministic row planner, dry-run/report design, and backend report tooling are complete; the next risk is evidence that the real non-zero balance candidates map or are refused cleanly in scratch without finance writes.
   - Likely files: no repo code changes for the execution itself; a later narrow docs/evidence refresh should record the artefact path and key totals if the run succeeds.
   - Likely validation: command exit code, JSON parse/top-level report shape, `import_ready=false`, manifest `no_write=true`, no R4 source mode unless a separate SELECT-only source artefact is explicitly used, no PMS DB writes, no finance records created or changed, and `git status --short`.
   - Backend-only/proof-only likely: yes; no importer wiring, staging model, apply mode, or write path.
   - Risk: high.
...
- PR #604 completed the backend-only pure opening-balance snapshot plan helper proof, and PR #607 completed backend-only opening-balance dry-run/report tooling with no R4 source mode, DB write path, or apply mode. The next finance slice is scratch-only dry-run execution evidence using the real PatientStats row artefact and scratch mapping evidence, with no importer, staging model, scratch write, or PMS finance write path.
