...
1. Cancellation pairing/refund-allocation mismatch proof.
   - Target: explain R4 cancellation/reversal rows and the refund mismatch between `vwPayments` refunds `110` and `PaymentAllocations`/`vwAllocatedPayments` refunds `795` without writing PMS finance records.
   - Why next: live opening balance evidence is complete, component checks passed, but refund/allocation mismatch and cancellation pairing remain explicit blockers before any finance import.
   - Likely files: backend SELECT-only/report-only helper or docs evidence depending on inspection; no importer, staging model, or PMS DB session.
   - Likely validation: focused report/schema tests if tooling is added, optional live SELECT-only artefact, no R4 writes, no PMS DB writes, no finance records created or changed.
...
- Finance classification/sign helper proof is complete as of PR #591, opening balance reconciliation command/report tooling is complete as of PR #593, and live opening balance proof evidence is recorded as of 2026-05-04.
- Next, prove cancellation pairing/refund-allocation mismatch semantics before any importer, staging model, or PMS finance write path.
