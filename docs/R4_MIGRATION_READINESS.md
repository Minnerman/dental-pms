...
1. Refund-allocation/charge-ref semantics decision.
   - Target: decide how to interpret PR #596 evidence before any finance import-readiness decision.
   - Why next: cancellation pairing is strong for `Adjustments CancellationOf` (`460/460` originals found, patient mismatches `0`, paired net `0.00`), but refund/allocation mismatch remains unresolved (`vwPayments` refunds `110`, `PaymentAllocations` refunds `795`, only `62` matching allocation refunds, `733` allocation refunds without `vwPayments` refund, `48` `vwPayments` refunds without allocation) and allocation rows still have `0` charge refs / `3130` missing charge refs.
   - Likely files: docs policy/update or pure helper/report tests depending on inspection; no importer, staging model, or PMS DB session.
   - Likely validation: docs diff checks or focused helper/report tests if code is added; no R4 writes, no PMS DB writes, no finance records created or changed.
   - Backend-only/proof-only or docs-only likely: yes.
   - Risk: high.
...
5. Finance payment method mapping helper continuation.
   - Target: after cancellation pairing/refund-allocation proof, tighten payment method and lookup mapping using the PR #591 classifier reason codes.
   - Why next later: payment method mapping matters for cash-up and reports, but cancellation/refund/allocation semantics are the higher-risk blocker.
...
- PR #596 proved `Adjustments CancellationOf` rows pair cleanly (`460/460`, paired net `0.00`), but refund counts still differ between `vwPayments` (`110`) and `PaymentAllocations`/`vwAllocatedPayments` (`795`); only `62` allocation refunds match `vwPayments` refunds, `733` allocation refunds lack a `vwPayments` refund, and `48` `vwPayments` refunds lack allocation rows.
- Allocation rows still have `0` charge refs and `3130` missing charge refs, blocking invoice application.
...
- Next, decide refund-allocation/charge-ref semantics before any importer, staging model, or PMS finance write path.
