...
| Past appointments | Yes: `dbo.vwAppointmentDetails`; live SELECT-only inventory records 101,051 appointments from 2001-10-27 to 2027-02-01, with 100,994 before the 2026-04-29 cutover date. | Yes: `r4_import --entity appointments` into `r4_appointments`. | Strong for raw R4 staging and promotion planning: linkage report/queue, manual mappings, unmapped appointments admin, live inventory evidence, PR #571 pure helper proof, the 2026-04-30 scratch transcript mapped `99299` rows with `1752` expected null-patient rows and `0` actionable unmapped rows, PR #574's no-core-write promotion dry-run/report classified the full `101051` rows, PR #576 added the pure promotion plan helper/proof, PR #578 added the timezone/local-time proof, PR #580 added the conflict predicate proof, and PR #582 added the guarded no-DB-write apply-plan prototype. | Strong for raw `r4_appointments` and no-core-write promotion classification: scratch import created `101051`, idempotency rerun created `0` and skipped `101051`; PR #574 found `94156` status-policy promote candidates and kept core `appointments` unchanged at `0` before/after; PR #576 made the row-level plan consumable without DB writes; PR #578 proves Europe/London local-wall-time conversion semantics without wiring importer/apply behaviour; PR #580 proves same-clinician conflict semantics without route or promotion wiring; PR #582 proves guarded apply planning without DB writes or runtime wiring. | Partial: read-only R4 calendar/admin linking plus core appointments UI. | Scratch-only CLI/runbook wiring and transcript that writes core appointments only inside an isolated scratch DB. | High | Medium-large |
| Future appointments | Yes: same `dbo.vwAppointmentDetails`; live inventory records 57 rows on/after 2026-04-29, including `Cancelled`, `Pending`, and `Deleted` examples through 2027-02-01. | Yes: same appointment importer. | Strong for no-core-write promotion planning: live inventory evidence, PR #571 fail-closed status helper tests, full scratch raw import/linkage evidence, PR #574 promotion dry-run evidence, PR #576 promotion plan helper/proof, PR #578 timezone/local-time proof, PR #580 conflict predicate proof, and PR #582 guarded apply-plan prototype exist; future core diary writes still require scratch-only execution proof before live/default use. | Strong for raw `r4_appointments` and no-core-write reporting/planning: PR #574 classified `47` future booked candidates, `7` future cancelled candidates, and `3` future deleted excluded rows without core writes; PR #576 proves deterministic row-level planning without importer/apply wiring; PR #578 proves naive R4 appointment datetimes as Europe/London clinic-local wall times, with invalid/ambiguous/non-existent times failing closed; PR #580 proves overlap predicate semantics for future guarded apply checks; PR #582 proves guarded apply planning without DB writes. | Partial: core diary, booking, R4 calendar read-only. | Scratch-only CLI/runbook wiring and transcript before active future diary promotion. | High | Medium-large |
...
4. Scratch-only guarded R4 appointment core-promotion CLI/runbook wiring and transcript, using PR #574's no-core-write report, PR #576's promotion plan helper, PR #578's timezone/local-time proof, PR #580's conflict predicate proof, and PR #582's guarded apply-plan prototype before any real core diary write path is enabled.
...
1. Scratch-only guarded core-promotion CLI/runbook wiring and transcript.
   - Target: wire the PR #582 guarded apply-plan prototype into an explicitly scratch-only execution path and capture a transcript that writes core appointments only inside an isolated scratch DB.
   - Why next: raw import, no-core-write promotion reporting, PR #576's pure promotion plan helper, PR #578's timezone/local-time proof, PR #580's conflict predicate proof, and PR #582's guarded apply-plan prototype are complete; actual scratch-only core `appointments` write evidence is the next unresolved prerequisite.
   - Likely files: narrowly scoped backend scratch-only CLI/runbook wiring and tests; do not enable live/default-DB core `appointments` writes.
   - Likely validation: scratch-first/default-DB refusal tests, confirmation token checks, dry-run-must-pass gate, core appointment before/after counts in scratch, created/skipped idempotency proof, git diff checks, no R4 writes, no live/default PMS DB writes.
...
- R4 status/cancelled/flag values now have inventory evidence, a design policy in `docs/r4/R4_APPOINTMENT_STATUS_POLICY.md`, PR #571 executable backend helper tests, a complete scratch raw `r4_appointments` import/idempotency/linkage transcript, PR #574's no-core-write promotion dry-run/report, PR #576's pure promotion plan helper/proof, PR #578's timezone/local-time proof, PR #580's conflict predicate proof, and PR #582's guarded no-DB-write apply-plan prototype; core diary promotion has not started.
- Null-patient appointments are known (`1752`) and must remain read-only/unlinked unless manually linked.
- Conflict predicate semantics are extracted/proven as of PR #580, and guarded apply planning is proven as of PR #582; future core diary promotion still needs scratch-only CLI/runbook execution proof before any real core writes.
