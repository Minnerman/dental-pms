...
1. Backend-only guarded scratch apply planning/preflight helper and tests.
   - Target: convert `docs/r4/R4_FINANCE_OPENING_BALANCE_GUARDED_APPLY_DESIGN.md` into executable no-write guard decisions for target refusal, dry-run report acceptance, manifest construction, idempotency keys, and planned ledger rows.
   - Why next: the guarded apply design is recorded, but a CLI or scratch execution should not exist until the apply gates are unit-proven without opening a DB write path.
   - Likely files: `backend/app/services/r4_import/opening_balance_snapshot_apply_plan.py`, `backend/tests/r4_import/test_opening_balance_snapshot_apply_plan.py`, and docs/status refresh.
   - Likely validation: focused backend tests, py_compile, no R4 access, no PMS DB writes, no finance records.
   - Backend-only/proof-only likely: yes; no execution.
   - Risk: high.
