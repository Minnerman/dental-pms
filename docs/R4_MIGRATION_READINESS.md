...
1. Guarded scratch apply CLI design/prototype, only after explicit instruction.
   - Target: design or prototype the guarded scratch-only opening-balance apply command surface around PR #612's proven preflight helper, still refusing default/live targets and still requiring scratch/test DB and `SCRATCH_OPENING_BALANCE_APPLY`.
   - Why next: PR #612 proves no-write apply gates, but no CLI, importer wiring, apply execution, DB session, or finance record write exists yet.
   - Likely files: a backend script/CLI plus focused tests only if explicitly requested; otherwise docs/design first.
   - Likely validation: focused backend tests, py_compile, explicit no R4 access, scratch-only DB target refusal checks, no PMS default/live writes, and no finance import.
   - Backend-only/proof-only likely: yes until an explicit scratch execution slice is requested.
   - Risk: high.
