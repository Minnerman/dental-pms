...
- `docs/r4/R4_FINANCE_OPENING_BALANCE_FULL_ELIGIBLE_ARTEFACT_PLAN.md` starts the later full eligible-row artefact planning track. It is planning-only: no R4 access, real artefact access, patient data, PMS DB connection, guarded scratch apply, CLI validation/no-write, live/default PMS write, actual PMS Postgres write, finance import, or full eligible-row artefact execution occurred. It defines future package requirements, approval gates, validation/no-write evidence, guarded apply/write evidence, rejection criteria, and stop conditions while keeping `finance_import_ready=false`.
...
1. Full eligible-row opening-balance artefact plan review.
   - Target: `docs/r4/R4_FINANCE_OPENING_BALANCE_FULL_ELIGIBLE_ARTEFACT_PLAN.md`.
   ...
   - Likely files: docs/status or a future provenance/redaction proposal only.
