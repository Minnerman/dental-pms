...
1. Bounded fixture scratch/test-only execution proof, only after explicit instruction.
   - Target: use the approved bounded fixture package in `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_PACKAGE.md` and the readiness report in `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_EXECUTION_READINESS.md` for a separately authorised scratch/test-only preserved-evidence execution slice; run validation/no-write first and run guarded apply only if that future slice explicitly authorises execution.
   - Why next: PR #615 proved the CLI write path and idempotency with synthetic non-R4 data, PR #616 defined the preserved-evidence execution plan, PR #617 selected Option B, PR #618 prepared the bounded fixture, PR #619 recorded owner approval for that exact package hash/checksum, and execution-readiness verification confirms the package and guard requirements are aligned for a later scratch/test-only slice.
   - Likely files: docs/evidence record for the validation/no-write and, if separately authorised, guarded scratch apply/idempotency proof; backend changes are not expected.
