...
1. Full eligible-row artefact/export decision before preserved-evidence scratch apply execution, only after explicit instruction.
   - Target: decide whether to produce a full eligible-row dry-run artefact from isolated scratch/test data or keep the next execution proof deliberately bounded.
   - Why next: this branch proves the CLI write path and idempotency with synthetic non-R4 data, but no preserved-evidence `1018`-row scratch apply execution has started and the preserved dry-run report still has bounded eligible samples.
   - Likely files: docs/evidence or a narrow dry-run artefact export helper only if explicitly authorised.
   - Likely validation: validation mode first, default/live refusal proof, isolated scratch/test-only apply, idempotency rerun, invoice/payment counts unchanged, stdout/stderr/exit-code/report artefacts, no R4 access unless separately authorised as SELECT-only, no live/default PMS writes, and no finance import.
