...
Baseline: `origin/master@4619a67a95e56dce5482678b1dea14e74330818a`
...
1. Review bounded fixture guarded apply/write evidence.
   - Target: review `docs/r4/R4_FINANCE_OPENING_BALANCE_BOUNDED_FIXTURE_GUARDED_APPLY_EVIDENCE.md` for the first local isolated scratch/test guarded apply/idempotency proof.
   - Why next: the authorised bounded fixture proof created `3` local scratch/test `PatientLedgerEntry` rows on the first run, then created `0` and skipped `3` on the idempotency run; invoices/payments stayed `0`, `finance_import_ready=false`, and no live/default PMS DB, actual PMS Postgres, R4, real artefact, real patient data, or finance import was used.
   - Likely files: docs/status review only unless the evidence review finds a specific blocker.
   - Likely validation: evidence hash review, fixture/manifest hash validation, manifest-scoped row count/total/reference review, idempotency result review, no real patient data in committed docs, no R4 access, no live/default PMS writes, and no finance import.
   - Backend-only/proof-only likely: yes; avoid live/default data and avoid implementation beyond any explicitly chosen fixture/package blocker.
   - Risk: high.
