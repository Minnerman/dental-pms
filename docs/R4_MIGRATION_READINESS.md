...
1. Pure opening-balance snapshot plan helper plus unit tests.
   - Target: encode deterministic row eligibility, sign, pence conversion, patient mapping requirements, and refusal reasons from `docs/r4/R4_FINANCE_OPENING_BALANCE_SNAPSHOT_DESIGN.md`.
   - Why next: the design is now recorded; the smallest next proof is a pure policy helper before any scratch DB dry-run or write path.
   - Likely files: `backend/app/services/r4_import/opening_balance_snapshot_plan.py`, `backend/tests/r4_import/test_opening_balance_snapshot_plan.py`.
   - Likely validation: focused pytest, existing finance classification/opening-balance tests if relevant, `git diff --check`; no R4 writes; no PMS DB writes; no finance records created or changed.
   - Backend-only/proof-only likely: yes; pure helper/test-only.
   - Risk: high.
...
- Next finance slice is a pure opening-balance snapshot plan helper plus unit tests before any importer, staging model, scratch write, or PMS finance write path.
