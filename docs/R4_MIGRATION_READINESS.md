...
| Past appointments | Yes: `dbo.vwAppointmentDetails`; live SELECT-only inventory records 101,051 appointments from 2001-10-27 to 2027-02-01, with 100,994 before the 2026-04-29 cutover date. | Yes: `r4_import --entity appointments` into `r4_appointments`. | Strong for raw R4 staging and promotion planning: linkage report/queue, manual mappings, unmapped appointments admin, live inventory evidence, PR #571 pure helper proof, the 2026-04-30 scratch transcript mapped `99299` rows with `1752` expected null-patient rows and `0` actionable unmapped rows, PR #574's no-core-write promotion dry-run/report classified the full `101051` rows, PR #576 added the pure promotion plan helper/proof, and PR #578 added the timezone/local-time proof. | Strong for raw `r4_appointments` and no-core-write promotion classification: scratch import created `101051`, idempotency rerun created `0` and skipped `101051`; PR #574 found `94156` status-policy promote candidates and kept core `appointments` unchanged at `0` before/after; PR #576 made the row-level plan consumable without DB writes; PR #578 proves Europe/London local-wall-time conversion semantics without wiring importer/apply behaviour. | Partial: read-only R4 calendar/admin linking plus core appointments UI. | Conflict predicate extraction before any guarded core-promotion apply path. | High | Medium-large |
| Future appointments | Yes: same `dbo.vwAppointmentDetails`; live inventory records 57 rows on/after 2026-04-29, including `Cancelled`, `Pending`, and `Deleted` examples through 2027-02-01. | Yes: same appointment importer. | Strong for no-core-write promotion planning: live inventory evidence, PR #571 fail-closed status helper tests, full scratch raw import/linkage evidence, PR #574 promotion dry-run evidence, PR #576 promotion plan helper/proof, and PR #578 timezone/local-time proof exist; future core diary writes still require conflict predicate and guarded apply proofs. | Strong for raw `r4_appointments` and no-core-write reporting: PR #574 classified `47` future booked candidates, `7` future cancelled candidates, and `3` future deleted excluded rows without core writes; PR #576 proves deterministic row-level planning without importer/apply wiring; PR #578 proves naive R4 appointment datetimes as Europe/London clinic-local wall times, with invalid/ambiguous/non-existent times failing closed. | Partial: core diary, booking, R4 calendar read-only. | Conflict predicate extraction before active future diary promotion or guarded core apply. | High | Medium-large |
...
4. R4 appointment conflict predicate extraction, using PR #574's no-core-write report, PR #576's promotion plan helper, and PR #578's timezone/local-time proof before any core diary write path is enabled.
...
1. Appointment conflict predicate extraction.
   - Target: extract/prove the pure overlap predicate future guarded apply code must use before any core diary write path is enabled.
   - Why next: raw import, no-core-write promotion reporting, PR #576's pure promotion plan helper, and PR #578's timezone/local-time proof are complete; conflict semantics are the next unresolved prerequisite for guarded promotion.
   - Likely files: narrowly scoped backend pure helper/tests around core appointment overlap rules; do not enable live/default-DB core `appointments` writes.
   - Likely validation: focused overlap fixtures for active/inactive statuses, adjacent boundaries, clinician presence, timezone-aware inputs, existing status/promotion plan/timezone tests, git diff checks, no R4 writes, no live/default PMS DB writes.
