...
- 2026-05-05: PR #607 merged on `master` as backend-only, proof-report-only, no-write opening-balance dry-run/report tooling.
  ...
  - Next finance slice: scratch-only dry-run execution evidence using the real `PatientStats` row artefact and scratch mapping evidence, no writes.
- 2026-05-05: Scratch-only opening-balance dry-run/report design recorded as docs/design-only.
  ...
  - Recommended next slice: superseded by PR #607's merged backend-only dry-run/report tooling; next finance slice is scratch-only dry-run execution evidence using the real `PatientStats` row artefact and scratch mapping evidence, no writes.
  - Safety: no R4 access occurred; no R4 writes occurred; no PMS DB writes occurred; no finance import/staging model was added; no invoices, payments, balances, ledger rows, or finance records were created or changed.
  - Import-readiness: `finance_import_ready=false`; finance import remains out of scope.
- 2026-05-05: PR #604 merged on `master` as backend-only, pure-helper/test-only opening-balance snapshot plan helper proof.
  ...
  - Import-readiness: `finance_import_ready=false`; superseded by PR #607's merged backend-only dry-run/report tooling; next finance slice is scratch-only dry-run execution evidence using the real `PatientStats` row artefact and scratch mapping evidence, no writes.
...
- 2026-05-04: Finance invoice/charge-ref source decision recorded after live cash-event staging proof.
  ...
  - Next finance slice: scratch-only dry-run execution evidence using the real `PatientStats` row artefact and scratch mapping evidence, no writes, before any finance import.
- 2026-05-04: Live SELECT-only finance cash-event staging proof completed with no tracked repo changes.
  ...
  - Superseded by the invoice/charge-ref source decision, opening-balance snapshot design, PR #604 plan helper proof, scratch dry-run/report design, and PR #607's merged dry-run/report tooling recorded above; next finance slice is scratch-only dry-run execution evidence using the real `PatientStats` row artefact and scratch mapping evidence, no writes.
