...
## Recent fixes
- 2026-05-06: Guarded scratch-only opening-balance apply CLI prototype added on a branch stacked on PR #613.
  ...
  - Validation: temporary venv py_compile passed; focused CLI/prototype tests passed (`8 passed`); adjacent opening-balance apply-plan, snapshot-plan, and dry-run tests passed with the new tests (`60 passed`).
  ...
  - Next finance slice: docs/status refresh after this PR if needed, then a guarded scratch-only apply execution proof only after explicit instruction and only with a full eligible-row artefact or explicitly bounded scratch fixture.
