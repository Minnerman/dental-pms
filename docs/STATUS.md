...
- The R4 finance track has completed source discovery, live SELECT-only inventory, fail-closed sign/cancellation/allocation policy, backend-only pure classification/sign helper proof, backend-only SELECT-only opening balance reconciliation command/report tooling, live SELECT-only opening balance proof evidence, live SELECT-only cancellation/refund/allocation reconciliation proof evidence, the refund-allocation/charge-ref semantics decision, and backend-only SELECT-only cash-event staging proof tooling with no importer behaviour changes, no R4 writes, no PMS DB writes, and no finance records created or changed. PR #587's inventory evidence path is `/home/amir/dental-pms-finance-inventory-proof/.run/finance_inventory_20260503_201724/`; opening balance evidence path is `/home/amir/dental-pms-opening-balance-live-proof/.run/opening_balance_reconciliation_20260504_083558/`; cancellation/allocation evidence path is `/home/amir/dental-pms-finance-cancellation-allocation-proof/.run/finance_cancellation_allocation_reconciliation_20260504_140810/`; the refund-allocation/charge-ref semantics decision is recorded in `docs/r4/R4_FINANCE_REFUND_ALLOCATION_CHARGE_REF_DECISION.md`; live cash-event staging proof evidence is still pending. Next finance slice is running and recording the live SELECT-only cash-event staging proof when complete R4 env is available.
...
- 2026-05-04: PR #599 merged on `master` as backend-only, SELECT-only cash-event staging proof/report tooling.
  ...
  - Live cash-event staging proof evidence has not yet been run because complete R4 env was not available during the implementation slice.
  ...
  - Finance import-readiness remains blocked by live cash-event evidence, refund/allocation mismatch, missing allocation charge refs, and payment/refund semantics risk.
  - Next finance slice: run and record the live SELECT-only cash-event staging proof when complete R4 env is available.
- 2026-05-04: Refund-allocation/charge-ref semantics decision recorded as docs/design-only in `docs/r4/R4_FINANCE_REFUND_ALLOCATION_CHARGE_REF_DECISION.md`.
  ...
  - Superseded by PR #599's merged SELECT-only cash-event staging proof tooling; next finance slice is the live SELECT-only cash-event staging proof run when complete R4 env is available.
- 2026-05-04: PR #596 merged on `master` as backend-only, SELECT-only cancellation/refund/allocation reconciliation proof tooling, with live SELECT-only evidence recorded.
  ...
  - Superseded by the refund-allocation/charge-ref semantics decision and PR #599's merged SELECT-only cash-event staging proof tooling recorded above; next finance slice is the live SELECT-only cash-event staging proof run when complete R4 env is available.
...
- 2026-05-04: PR #593 merged on `master` as backend-only, SELECT-only opening balance reconciliation command/report tooling.
  ...
  - Superseded by the live SELECT-only opening balance proof evidence, PR #596 cancellation/refund/allocation proof evidence, the refund-allocation/charge-ref semantics decision, and PR #599's merged SELECT-only cash-event staging proof tooling recorded above; next finance slice is the live SELECT-only cash-event staging proof run when complete R4 env is available.
