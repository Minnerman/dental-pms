...
- The R4 finance track has completed source discovery, live SELECT-only inventory, fail-closed sign/cancellation/allocation policy, backend-only pure classification/sign helper proof, and backend-only SELECT-only opening balance reconciliation command/report tooling with no importer behaviour changes, no R4 writes, no PMS DB writes, and no finance records created or changed. PR #587's inventory evidence path is `/home/amir/dental-pms-finance-inventory-proof/.run/finance_inventory_20260503_201724/`; live opening balance proof evidence is still pending, and the next slice is to run/record the live SELECT-only opening balance proof when complete R4 env is available.
...
- 2026-05-04: PR #593 merged on `master` as backend-only, SELECT-only opening balance reconciliation command/report tooling.
  ...
  - Next finance slice: run/record the live SELECT-only opening balance proof when complete R4 env is available, then continue to cancellation pairing/refund-allocation mismatch proof.
