...
- 2026-05-04: Opening-balance snapshot import design recorded as docs/design-only.
  ...
  - Future scratch proof recommendation: start with a pure opening-balance snapshot plan helper plus unit tests; later scratch proof may dry-run first and only then, with separate approval, write one manifest-scoped `PatientLedgerEntry` adjustment per eligible non-zero mapped patient in scratch only.
  ...
  - Import-readiness: `finance_import_ready=false`; next finance slice is a pure opening-balance snapshot plan helper plus unit tests.
- 2026-05-04: Finance invoice/charge-ref source decision recorded after live cash-event staging proof.
  ...
  - Next finance slice: pure opening-balance snapshot plan helper plus unit tests before any finance import.
- 2026-05-04: Live SELECT-only finance cash-event staging proof completed with no tracked repo changes.
  ...
  - Superseded by the invoice/charge-ref source decision and opening-balance snapshot design recorded above; next finance slice is a pure opening-balance snapshot plan helper plus unit tests before any finance import.
...
- 2026-05-04: Refund-allocation/charge-ref semantics decision recorded as docs/design-only in `docs/r4/R4_FINANCE_REFUND_ALLOCATION_CHARGE_REF_DECISION.md`.
  ...
  - Superseded by PR #599's merged SELECT-only cash-event staging proof tooling, the live cash-event proof evidence, the invoice/charge-ref source decision, and the opening-balance snapshot design recorded above; next finance slice is a pure opening-balance snapshot plan helper plus unit tests before any finance import.
- 2026-05-04: PR #596 merged on `master` as backend-only, SELECT-only cancellation/refund/allocation reconciliation proof tooling, with live SELECT-only evidence recorded.
  ...
  - Safety: R4 access was SELECT-only; no R4 writes occurred; no PMS DB writes occurred; no finance import/staging model was added; no invoices, payments, balances, ledger rows, or finance records were created or changed.
  - Superseded by the refund-allocation/charge-ref semantics decision, PR #599's merged SELECT-only cash-event staging proof tooling, the live cash-event proof evidence, the invoice/charge-ref source decision, and the opening-balance snapshot design recorded above; next finance slice is a pure opening-balance snapshot plan helper plus unit tests before any finance import.
...
- 2026-05-04: PR #593 merged on `master` as backend-only, SELECT-only opening balance reconciliation command/report tooling.
  ...
  - Superseded by the live SELECT-only opening balance proof evidence, PR #596 cancellation/refund/allocation proof evidence, the refund-allocation/charge-ref semantics decision, PR #599's merged SELECT-only cash-event staging proof tooling, the live cash-event proof evidence, the invoice/charge-ref source decision, and the opening-balance snapshot design recorded above; next finance slice is a pure opening-balance snapshot plan helper plus unit tests before any finance import.
